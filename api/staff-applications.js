...
import { readSession, canHandle } from "./_lib/auth.js";
...
export default async function handler(req, res) {
  ...
  if (req.method === "GET") {
    ...
    if (!canHandle(requestedType, session.roles || [])) {
  ...
  if (req.method === "PATCH") {
    ...
    if (!id || !validStatus.has(status)) {
      return res.status(400).json({ error: "Ugyldige data." });
    }

    const existing = await sql`
      SELECT type, discord_id, status
      FROM applications
      WHERE id = ${id}
      LIMIT 1
    `;
    ...
    if (!canHandle(type, session.roles || [])) {
      return res.status(403).json({ error: "Ingen adgang." });
    }
    ...
    if (type === "whitelist" && previousStatus !== status) {
      if (status === "approved") {
        const webhookResult = await sendWhitelistWebhook(
          applicantDiscordId,
          "Din Whitelist ansøgning er Godkendt"
        );

        if (!webhookResult.ok) {
          console.error(
            "Godkendt-besked blev ikke sendt:",
            webhookResult.error
          );
        }
      }
      ...
      if (status === "rejected") {
        const webhookResult = await sendWhitelistWebhook(
          applicantDiscordId,
          "Din Whitelist Ansøgning er Afvist."
        );

        if (!webhookResult.ok) {
          console.error(
            "Afvist-besked blev ikke sendt:",
            webhookResult.error
          );
        }
      }
